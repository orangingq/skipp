...
from models.torchgpipe_balance.microbatch import Batch
...
def profile_sizes(module: nn.Sequential,
    ...
    for layer in layerwise_sandbox(module, device):
        detach(batch)
        layer.eval() # CSH : added to avoid batchnorm error
        ...
        param_size = sum(p.untyped_storage().size() * p.untyped_storage().element_size()
